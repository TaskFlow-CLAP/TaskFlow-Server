...
public class TaskBoardController {
    ...
    public ResponseEntity<TaskBoardResponse> getTaskBoard(@RequestParam(defaultValue = "0") int page,
                                                          ...
                                                          @ModelAttribute @Valid FilterTaskBoardRequest request,
                                                          @AuthenticationPrincipal SecurityUserDetails userInfo) {
        Pageable pageable = PageRequest.of(page, pageSize);
        if (request != null) {
            return ResponseEntity.ok(filterTaskBoardUsecase.getTaskBoardByFilter(userInfo.getUserId(), fromDate, request, pageable));
        } else
            return ResponseEntity.ok(getTaskBoardUsecase.getTaskBoards(userInfo.getUserId(), fromDate, pageable));
    }
